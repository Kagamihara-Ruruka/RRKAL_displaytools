...
from scripts.validate_layer_render_plan_adapter_preflight_import_boundary import (
    validate_source,
    validate_target,
)
...
SCRIPT = REPO_ROOT / "scripts" / "validate_layer_render_plan_adapter_preflight_import_boundary.py"
...
class LayerRenderPlanAdapterPreflightImportBoundaryTests(unittest.TestCase):
    ...
    def test_safe_adapter_preflight_source_passes(self):
        source = (
            ...
            "\n"
            "def build_layer_render_plan_adapter_payload(runtime_snapshot, composition_steps):\n"
            "    return {'runtime_snapshot': runtime_snapshot, 'composition_steps': composition_steps}\n"
            "\n"
            "def build_layer_render_plan_adapter_payload_contract(payload):\n"
            "    return {'status': 'ready' if payload else 'incomplete'}\n"
        )
    ...
    def test_controller_alpha_apply_path_compiled_and_batch_imports_fail(self):
        snippets = [
            ...
            "from render_core.render_plan import build_reused_compiled_layer_render_plan_packet_from_adapter_payload\n",
    ...
    def test_sibling_render_plan_module_imports_fail(self):
        snippets = [
            "from render_core.render_plan import build_layer_render_plan_adapter_payload\n",
    ...
    def test_cli_forbidden_file_exits_nonzero_with_json(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            target = Path(tmpdir) / "bad_adapter_preflight.py"
            target.write_text("from render_core.render_plan import build_compiled_layer_render_plan_packet\n", encoding="utf-8")
            result = subprocess.run(
