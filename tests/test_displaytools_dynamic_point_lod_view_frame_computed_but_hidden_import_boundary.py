"""Tests for the dynamic point computed-but-hidden import-boundary checker."""
...
from scripts import validate_displaytools_dynamic_point_computed_but_hidden_import_boundary as checker


class DynamicPointComputedButHiddenImportBoundaryTest(unittest.TestCase):
    def test_default_target_is_computed_but_hidden_boundary(self) -> None:
        self.assertEqual(checker.DEFAULT_TARGET, Path("render_core/dynamic_point_computed_but_hidden_boundary.py"))
    ...
    def test_allowed_label_as_executable_reference_fails(self) -> None:
        snippets = [
            ...
            "def computed_point_token():\n    return None\n",
            "class hidden_visibility_token:\n    pass\n",
            "def f(obj):\n    return obj.frame_visible_not_observed\n",
            "def f():\n    return computed_but_hidden_contract()\n",
        ]
    ...
    def test_forbidden_imports_fail(self) -> None:
        ...
        packet = checker.validate_source("from render_core.render_plan import alpha_compose\n")
        self.assertFalse(packet["boundary_passed"])
        self.assertIn("alpha_compose_formula", {row["forbidden_family"] for row in packet["violations"]})
    ...
    def test_forbidden_names_fail(self) -> None:
        snippets = {
            ...
            "source_lineage_loss_interpretation": "value = source_lineage_loss_claimed\n",
    ...
    def test_checker_does_not_import_target(self) -> None:
        ...
        self.assertNotIn("render_core.dynamic_point_computed_but_hidden_boundary", imported_modules)
        self.assertNotIn("dynamic_point_computed_but_hidden_boundary", imported_modules)
    ...
    def test_cli_missing_target_self_test_and_syntax_error(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            completed = subprocess.run(
                [sys.executable, "-B", "scripts/validate_displaytools_dynamic_point_computed_but_hidden_import_boundary.py", str(missing)],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            failed = subprocess.run(
                [sys.executable, "-B", "scripts/validate_displaytools_dynamic_point_computed_but_hidden_import_boundary.py", str(syntax_error)],
                capture_output=True,
                text=True,
            )
        ...
        completed = subprocess.run(
            [sys.executable, "-B", "scripts/validate_displaytools_dynamic_point_computed_but_hidden_import_boundary.py", "--self-test-negative"],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_module_can_import_safely(self) -> None:
        spec = importlib.util.spec_from_file_location(
            "computed_but_hidden_checker_safe_import",
        ...
        self.assertEqual(str(module.DEFAULT_TARGET), "render_core\\dynamic_point_computed_but_hidden_boundary.py")
