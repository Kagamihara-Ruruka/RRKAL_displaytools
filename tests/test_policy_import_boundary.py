...
from scripts.validate_policy_import_boundary import validate_source, validate_target


REPO_ROOT = Path(__file__).resolve().parents[1]
SCRIPT = REPO_ROOT / "scripts" / "validate_policy_import_boundary.py"


class PolicyImportBoundaryTests(unittest.TestCase):
    def test_missing_candidate_returns_json_pass(self):
        packet = validate_target(REPO_ROOT / "render_core" / "missing_point_overlay_budget_policy.py")
    ...
    def test_current_candidate_passes_import_boundary(self):
        packet = validate_target(REPO_ROOT / "render_core" / "point_overlay_budget_policy.py")
    ...
    def test_cli_missing_candidate_output_is_json(self):
        result = subprocess.run(
            [sys.executable, "-B", str(SCRIPT), str(REPO_ROOT / "render_core" / "missing_point_overlay_budget_policy.py")],
            cwd=REPO_ROOT,
            check=True,
            capture_output=True,
            text=True,
    ...
    def test_cli_forbidden_file_exits_nonzero_and_outputs_json(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            target = Path(tmpdir) / "bad_policy.py"
    ...
    def test_parser_normalizer_provider_imports_fail(self):
        snippets = [
