...
from scripts.validate_layer_render_plan_execution_phase_timing_import_boundary import (
    validate_source,
    validate_target,
)
...
SCRIPT = REPO_ROOT / "scripts" / "validate_layer_render_plan_execution_phase_timing_import_boundary.py"
...
class LayerRenderPlanExecutionPhaseTimingImportBoundaryTests(unittest.TestCase):
    ...
    def test_safe_execution_phase_timing_source_passes(self):
        source = (
            ...
            "\n"
            "def build_layer_render_plan_execution_summary(apply_path, batch_decisions):\n"
            "    return {'apply_path_count': len(apply_path), 'batch_decision_count': len(batch_decisions)}\n"
            "\n"
            "def build_layer_render_plan_phase_timing_runtime_packet(phase_timing_ms):\n"
            "    return {'phase_timing_ms': dict(phase_timing_ms), 'runtime_measurements_available': bool(phase_timing_ms)}\n"
        )
    ...
    def test_render_plan_sibling_modules_and_apply_path_fail(self):
        snippets = [
            "from render_core.render_plan import build_layer_render_plan_execution_summary\n",
    ...
    def test_alpha_metadata_artifact_parser_normalizer_policy_imports_fail(self):
        snippets = [
    ...
    def test_cli_forbidden_file_exits_nonzero_with_json(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            target = Path(tmpdir) / "bad_execution_phase_timing.py"
            target.write_text("from render_core.render_plan import build_layer_render_plan_apply_path\n", encoding="utf-8")
            result = subprocess.run(
