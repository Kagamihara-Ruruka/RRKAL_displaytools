...
| `build_layer_render_plan_composition_apply_action` | `render_core/layer_render_plan_composition_dispatch.py:def build_layer_render_plan_composition_apply_action`; re-exported by `render_core.render_plan` | `pure_helper_candidate` | step dict only | yes | no | medium because it maps kind to apply helper names | low | consumed by minimal dispatch helper movement; future work must not expand into composition execution |
| `build_layer_render_plan_composition_dispatch_packet` | `render_core/layer_render_plan_composition_dispatch.py:def build_layer_render_plan_composition_dispatch_packet`; re-exported by `render_core.render_plan` | `pure_helper_candidate` | action dict and overlay-present flag | yes | no | medium because dispatch branches map to composition execution | low | consumed by minimal dispatch helper movement; packet labels must not imply pixel equivalence |
...
| `build_layer_render_plan_cache_key` | `render_core/layer_render_plan_cache_diagnostics.py:def build_layer_render_plan_cache_key`; re-exported by `render_core.render_plan` | `pure_helper_candidate` | runtime snapshot, composition steps, style/blend/opacity maps | yes | no | low | medium | consumed by minimal cache diagnostics movement |
| `build_layer_render_plan_cache_invalidation_reasons` | `render_core/layer_render_plan_cache_diagnostics.py:def build_layer_render_plan_cache_invalidation_reasons`; re-exported by `render_core.render_plan` | `pure_helper_candidate` | runtime snapshot and cache key state | yes | no | low | medium | consumed by minimal cache diagnostics movement |
