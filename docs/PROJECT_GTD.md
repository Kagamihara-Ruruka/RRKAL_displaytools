...
| Display shell / canvas matrix | In progress | `display_core` now defines renderer-package-free `LayerModel`, `ViewModel`, `CanvasDescriptor`, `RendererEntry`, `register_renderer`, `lookup_renderers`, `build_canvas_registry_packet`, `build_sample_view_models_packet`, `build_view_model_render_plan_packet`, `build_sample_view_model_render_plans_packet` and `build_display_shell_capability_packet` contracts. `display_runtime/earth_canvas.py` now exists as the future EarthCanvas wrapper landing zone with `rrkal_displaytools.earth_canvas_runtime_contract.v1` but still does not move Taichi runtime code; `display_runtime/time_series_canvas.py` mirrors that shape for the first non-Earth canvas with `rrkal_displaytools.time_series_canvas_runtime_contract.v1` and no chart backend import; `display_runtime/protocols.py` defines the renderer-package-free runtime request/result and `CanvasRuntimeAdapter` protocol; `display_runtime/samples.py` bridges sample ViewModel render plans into `rrkal_displaytools.sample_canvas_runtime_requests.v1`; `scripts/export_display_runtime_contracts.py` exports both landing zones in one packet, `scripts/check_display_runtime_contracts.ps1` gates the packet including sample runtime requests, `scripts/inspect_display_runtime_handoff.ps1` aggregates display shell to runtime handoff readiness, and `scripts/check_display_runtime_import_boundary.ps1` keeps those skeletons backend-free. `scripts/inspect_display_shell_render_matrix.ps1` gives clone-first evidence for the DisplayShell + Canvas System + Layer System + Render Matrix direction without replacing the current globe runtime; `scripts/export_display_shell_render_matrix.py` exports the actual headless capability packet, `scripts/check_display_shell_render_matrix.ps1` gives a pass/fail gate, `scripts/check_display_core_import_boundary.ps1` guards the renderer-package-free boundary, and `scripts/inspect_earth_canvas_runtime_boundary.ps1` maps the current globe runtime owner plus future EarthCanvas extraction seam; all are listed in the no-GUI reviewer index. `rrkal_displaytools.canvas_registry.v1` lists phase-1 `EarthCanvas` and `TimeSeriesCanvas` boundaries, `rrkal_displaytools.sample_view_models.v1` proves Earth / TimeSeries views share the same ViewModel formula, `rrkal_displaytools.view_model_render_plan.v1` proves pure metadata dispatch plan selection, and contract-only `DisplayToolsGeoRendererAdapter` / `ContractOnlyTimeSeriesRendererAdapter` samples prove decorator registry discovery. | After render-plan composition seams stabilize, extract the current globe path as `EarthCanvas`, then add a minimal `TimeSeriesCanvas` adapter before broad renderer adapters. |
| Module boundaries | In progress | Monolith contains diagnostics for extraction readiness and seam matrices. `decoupling_readiness.py` records the `2026-05-31T07:00:00+08:00` not-before gate, post-7 extraction order, controlled interception policy, observability baseline, boundary inspector, render-plan compose work order and snapshot command; Qt/no-GUI reviewer packets expose it; Qt Replay/contracts exposes the snapshot command; `scripts/pre_decoupling_gate.ps1` verifies the first extraction, clean worktree, UIUX readiness, boundary inspector, render-plan compose work order and performance telemetry contract before code moves, and refuses formal execution before the 7:00 not-before timestamp. The pre-decoupling snapshot now embeds UIUX readiness, boundary inspection and render-plan compose work order for one-file handoff; `scripts/export_pre_decoupling_readiness_bundle.ps1` exposes the same readiness state plus embedded UIUX readiness without writing state; `scripts/check_pre_decoupling_readiness.ps1` gives a compact pass/fail check that also verifies the UIUX readiness pre-move requirement and embedded UIUX pass result; `scripts/inspect_render_plan_extraction_dry_run.ps1` gives a no-code-move checklist for the first post-7 extraction; `scripts/inspect_render_plan_compose_source_map.ps1` maps the first extraction helper lines across `taichi_global_bathymetry.py` and `render_core/render_plan.py`; `render_core/render_plan.py` now owns alpha compose helpers, compiled plan packet builder, reused compiled plan refresh helper, from-payload compiled/reused builder wrappers, runtime snapshot builder, composition step builder, composition input selector, composition apply action descriptor, composition dispatch packet, composition timing packet, style postprocess packet, compose queue classifier, compose queue packet builder, compose queue packet-from-states orchestrator, compose-run builder, compose parity contract builder, single-pass preflight contract builder, adapter boundary contract builder, adapter payload builder, adapter payload completeness contract builder, adapter payload summary builder, apply-path builder, execution summary builder, execution phases builder, phase timing contract builder, bottleneck recommendation builder, phase timing runtime packet builder, metadata summary builder, cache key builder, cache invalidation reasons builder, cache invalidation scope builder, batch decisions builder and step runtime state packet builder; runtime snapshot, composition step, composition input selector, composition apply action descriptor, composition dispatch packet, composition timing packet, style postprocess packet and compose queue packet-from-states helper ownership now points to render-core while controller wrappers are explicitly treated as runtime fact / boundary scalar / overlay lookup input collectors; compile path now passes precomputed composition steps into the runtime snapshot instead of rebuilding them; residual controller wrappers for compose-run/parity, runtime phase timing packet and pure cache/batch/apply/execution decision helpers were retired, while the monolith still preserves runtime overlay lookup and composition application contracts; `render_core/render_plan_performance.py` now owns the shared `layer_render_plan_performance_packet` handoff/capability contract to prevent renderer/Qt/no-GUI exporter drift; `docs/MODULE_BOUNDARIES.zh-TW.md` records this concrete seam and keeps overlay ndarray / Qt / IO responsibilities in the controller boundary. | Continue extracting render-plan compose helpers in small smoke-gated slices. |
...
| Layer render-plan performance | Planned | `layer_render_plan_performance` records the precompute-layer-state-then-single-render-pass target; `scripts/inspect_layer_render_plan_performance.ps1` now exposes the compose pass budget, merge preflight, disabled runtime merge and zero-diff parity requirements for clone-first review. `scripts/inspect_render_plan_single_pass_preflight.ps1` verifies the source-visible disabled single-pass gate, parity smoke gate, measured phase timing gate and manual review gate without launching Qt/Taichi. `scripts/inspect_render_plan_metadata_summary.ps1` verifies that renderer metadata preserves the full `layer_render_plan`, exposes `layer_render_plan_summary`, and surfaces `adapter_payload_status` plus `adapter_payload_contract_status` without launching Qt/Taichi or writing metadata. `scripts/inspect_render_plan_review_packet.ps1` aggregates these inspectors into one clone-first render-plan decoupling review entrypoint, including adapter payload and adapter payload contract status visibility. Renderer capability/handoff now also advertises the adapter payload schema, summary field, completeness contract status field and primary-gate marker. | After 7:00 decoupling, extract render-plan compose into `render_core/render_plan.py` before enabling any collapsed compose-run candidate. |
