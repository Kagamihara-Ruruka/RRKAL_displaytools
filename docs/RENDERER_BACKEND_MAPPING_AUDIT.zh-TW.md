...
| Metadata summary | `write_output_metadata()` and `render_core.layer_render_plan_cache_diagnostics.build_layer_render_plan_metadata_summary()` re-exported by `render_core.render_plan` | Uses compiled layer render plan or compiles one if missing | All visible layers and selected layer state | `layer_render_plan_summary`, full `layer_render_plan` | Contains phase timing runtime and bottleneck recommendation when available | `.metadata.json` sidecar | Summary builder is physically separated; sidecar writer remains unchanged and should be handled separately |
