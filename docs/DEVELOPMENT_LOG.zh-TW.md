...
- Kept the visible runtime gate strip concise while making the clipboard handoff text complete.
- Added smoke coverage for the combined copy handler text.
- Smoke: PASS (`powershell -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_displaytools\scripts\smoke.ps1`).
...
- Summarized the required baseline/candidate artifacts, visual parity requirement and zero-diff tolerances in one reviewer-facing line.
- Added smoke gates for the contract and runtime review packet summary text.
- Smoke: PASS (`powershell -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_displaytools\scripts\smoke.ps1`).
...
- Updated `Inspect: Render plan perf` to refresh the runtime optimization summary strip.
- Added `runtime_optimization_review_summary_text` to the Qt render-plan performance payload.
- Smoke: PASS after transient file-access backoff retries (`powershell -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_displaytools\scripts\smoke.ps1`).
...
- Added the import boundary check to the visual contract inspector index and review packet first-command list.
- Smoke now verifies the import boundary check schema, pass status and reviewer index entry.
- Smoke: PASS after transient file-access backoff retries (`powershell -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_displaytools\scripts\smoke.ps1`).
...
- Updated `HybridRenderController.apply_layer_render_plan_composition` to store a timing packet while preserving `layer_render_step_timing_ms` as the existing phase timing dict used by the render path.
- Added `composition_timing_packet_helper` to launch/capability/handoff/work-order smoke contracts.
- Smoke: PASS after transient file-access backoff retry (`powershell -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_displaytools\scripts\smoke.ps1`).

## 2026-05-31 - Extract composition dispatch packet

- Added `build_layer_render_plan_composition_dispatch_packet` to `render_core/render_plan.py` for pure overlay-present, skip-reason and dispatch-branch selection.
- Updated sequential and merged-candidate composition paths to consume the dispatch packet while keeping all ndarray composition helper calls in the controller.
- Added `composition_dispatch_packet_helper` to launch/capability/handoff/work-order smoke contracts.
- Smoke: PASS after transient file-access backoff retry (`powershell -NoProfile -ExecutionPolicy Bypass -File L:\RRKAL_displaytools\scripts\smoke.ps1`).
...
- Updated `HybridRenderController.layer_render_plan_phase_timing_runtime_packet` to delegate to the extracted helper.
- Kept runtime packet schema/source fields stable and reused the extracted bottleneck recommendation builder.
- Smoke: PASS (`powershell -NoProfile -ExecutionPolicy Bypass -File scripts\smoke.ps1`).
...
- Updated `HybridRenderController.layer_render_plan_execution_phases` to delegate to the extracted pure helper.
- Updated source-map and smoke gates for phase markers in render-core.
- Smoke: PASS (`powershell -NoProfile -ExecutionPolicy Bypass -File scripts\smoke.ps1`).
...
- Updated `HybridRenderController.layer_render_plan_compose_run_parity_contract` to delegate to the extracted helper while preserving the existing `source` field.
- Updated source-map and smoke source gates for the extracted parity markers.
- Smoke: PASS (`powershell -NoProfile -ExecutionPolicy Bypass -File scripts\smoke.ps1`).
