...
| `AdaptiveRenderQualityPolicy` | `5069` | `policy_object_candidate` | width/height/render_ms/lod/target fps; output decision dict/text | none observed | no | no | no | no | yes | Moved to `render_core/adaptive_render_quality_policy.py` after fixture and import-boundary gates; timing semantics still need care for future changes. |
