...
| `LayerRenderBudgetPolicy` | `5086` | `policy_object_candidate` | canvas size/render timing/lod/target fps/interaction/layer visibility; output decision dict/text | `LAYER_RENDER_COSTS` | no | no | no | no | yes | Current gates: `LAYER_RENDER_BUDGET_POLICY_FIXTURE_PARITY_GATE.zh-TW.md` and `LAYER_RENDER_BUDGET_POLICY_MOVEMENT_PREIMPLEMENTATION_GATE.zh-TW.md`; higher risk because cost map movement must be reviewed. |
