...
| `LayerRenderBudgetPolicy` | `5086` | `policy_object_candidate` | canvas size/render timing/lod/target fps/interaction/layer visibility; output decision dict/text | `LAYER_RENDER_COSTS` | no | no | no | no | yes | Moved with `LAYER_RENDER_COSTS` to `render_core/layer_render_budget_policy.py` after fixture and import-boundary gates; future changes still require parity review. |
...
| `AdaptiveRenderQualityPolicy` | `5069` | `policy_object_candidate` | width/height/render_ms/lod/target fps; output decision dict/text | none observed | no | no | no | no | yes | Current gates: `ADAPTIVE_RENDER_QUALITY_POLICY_FIXTURE_PARITY_GATE.zh-TW.md` and `ADAPTIVE_RENDER_QUALITY_POLICY_MOVEMENT_PREIMPLEMENTATION_GATE.zh-TW.md`; timing semantics need care. |
