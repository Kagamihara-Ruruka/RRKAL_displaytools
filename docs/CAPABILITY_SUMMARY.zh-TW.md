...
- Renderer layer runtime sync 下一步擴充 opacity、blend mode、renderer-side lock enforcement；目前 visibility 已可透過 `state/renderer_layer_runtime_state.json` 即時同步，Qt 端已有 bridge diagnostics、history、renderer ack 與 lock 防誤改。
