...
- `rrkal_displaytools.runtime_pressure_snapshot.v1` 已接到 renderer metadata 的 `policies.runtime_pressure_snapshot`，會回報 last render ms、target FPS/frame budget、pressure、interaction active、LOD、visible layer count、vector record count 與 heavy overlay defer/cache counters；它只做觀測，不改 render loop。
- `rrkal_displaytools.layer_render_state_packet.v1` 已接到 renderer metadata 的 `policies.layer_render_state`，會凍結目前 layer visibility、opacity、blend mode、dirty flags、renderer target、LOD bucket 與 defer reason，作為後續先編譯 LayerRenderState 再單次提交 render plan 的證據形狀。
- `rrkal_displaytools.lod_counter_packet.v1` 已接到 renderer metadata 的 `policies.lod_counters`，把目前 LOD bucket、visible vector records、deferred overlay count、cache hit/miss、target FPS pressure 放在同一個 packet，方便後續判斷 slow frame 是 LOD、cache 或 heavy overlay 造成。
...
- Qt Layers panel 現在有 `runtimeOptimizationReviewSummaryStrip`；執行 `Inspect: Render plan perf` 會把 runtime optimization summary contract 摘要顯示在面板上。
