...
- Layer capability matrix 已建立為 `rrkal_displaytools.layer_capability_matrix.v1`，會列出每個 Qt layer 的 renderer target、visibility/opacity/blend/selected-layer pick 是否 live、live control counts 與 selected layer capabilities。Qt Properties 會直接顯示選取圖層能力與最近 renderer ack runtime status，Layers dock 可顯示完整 JSON；launch packet、No-GUI export、renderer capabilities、handoff inspection 與 smoke gate 也會驗證此 contract。矩陣內含 `rrkal_displaytools.layer_runtime_evidence.v1`，Qt runtime 會摘要 changed visibility/opacity/blend、skipped locked layers、selected renderer target、event/error/frame；No-GUI 與 static renderer capability discovery 會明確標示 runtime evidence unavailable。
...
- Layer capability matrix 下一步可把 renderer ack evidence 拆到每個圖層的 UI badge，讓研究者不用打開 JSON 也能看到最後一次 apply 結果。
...
- Smoke 目前檢查：
  ...
  - Launch packet `layer_runtime_evidence` schema gate。
