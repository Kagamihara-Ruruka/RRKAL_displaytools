"""Validate import boundaries for a future compiled/reused packet helper.
...
SCHEMA = "rrkal_displaytools.layer_render_plan_compiled_reused_packets_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/layer_render_plan_compiled_reused_packets.py")
...
FORBIDDEN_IMPORT_PREFIXES = (
    ...
    "render_core.layer_render_plan_execution_phase_timing",
    "render_core.layer_render_plan_adapter_preflight",
    "render_core.preview",
...
FORBIDDEN_NAME_IMPORTS = {
    ...
    "compose_runtime_overlay",
    "compose_runtime_blend",
    "alpha_compose",
    "alpha_blend_compose",
...
def _name_is_forbidden(name: str) -> tuple[bool, str]:
    if name in FORBIDDEN_NAME_IMPORTS:
...
def validate_source(source: str, source_name: str = "<memory>") -> dict[str, object]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            name_hits = _forbidden_name_hits(_imported_names(node))
    ...
    return {
        ...
        "allowed_future_helper_behavior": "pure_compiled_reused_dict_list_scalar_packet_assembly_only",
        "cache_metadata_field_names_allowed_as_data": True,
        "cache_lifecycle_imports_forbidden": True,
        "metadata_artifact_writers_forbidden": True,
        "alpha_apply_batch_behavior_excluded": True,
...
def validate_target(path: Path) -> dict[str, object]:
    if not path.exists():
        return {
            ...
            "metadata_artifact_writers_forbidden": True,
            "alpha_apply_batch_behavior_excluded": True,
            "boundary": "Candidate compiled/reused packet helper module is not present yet; no target module was imported or executed.",
        }
...
def run_negative_self_test() -> dict[str, object]:
    snippets = {
        ...
        "pyais": "import pyais\n",
        "render_plan_module": "from render_core.render_plan import build_compiled_layer_render_plan_packet\n",
        ...
        "execution_phase_timing": "from render_core.layer_render_plan_execution_phase_timing import build_layer_render_plan_execution_summary\n",
        "adapter_preflight": "from render_core.layer_render_plan_adapter_preflight import build_layer_render_plan_adapter_payload\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate compiled/reused packet helper import boundaries."
    )
    ...
    packet = (
        ...
        else validate_target(Path(args.target))
    )
