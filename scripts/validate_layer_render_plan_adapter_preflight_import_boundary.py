"""Validate import boundaries for a future adapter/preflight helper module.
...
SCHEMA = "rrkal_displaytools.layer_render_plan_adapter_preflight_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/layer_render_plan_adapter_preflight.py")
...
def validate_source(source: str, source_name: str = "<memory>") -> dict[str, object]:
    ...
    return {
        ...
        "allowed_future_helper_behavior": "pure_adapter_preflight_dict_list_scalar_packet_logic_only",
        "compiled_reused_builders_excluded": True,
        "alpha_apply_path_excluded": True,
...
def validate_target(path: Path) -> dict[str, object]:
    if not path.exists():
        return {
            ...
            "compiled_reused_builders_excluded": True,
            "alpha_apply_path_excluded": True,
            "boundary": "Candidate adapter/preflight helper module is not present yet; no target module was imported or executed.",
        }
...
def run_negative_self_test() -> dict[str, object]:
    snippets = {
        ...
        "pyais": "import pyais\n",
        "render_plan_module": "from render_core.render_plan import build_layer_render_plan_adapter_payload\n",
        ...
        "compiled_builder": "from render_core.render_plan import build_compiled_layer_render_plan_packet\n",
        "compiled_builder_name": "def f():\n    return build_reused_compiled_layer_render_plan_packet\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate adapter/preflight helper import boundaries.")
