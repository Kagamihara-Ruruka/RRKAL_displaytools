"""Validate import boundaries for dynamic point source-lineage guard helpers.
...
SCHEMA = "rrkal_displaytools.dynamic_point_source_lineage_guard_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/dynamic_point_source_lineage_guard_boundary.py")

FORBIDDEN_FAMILIES: dict[str, tuple[str, ...]] = {
    "monolith": ("taichi_global_bathymetry",),
    "runtime_probe": ("dynamic_point_lod_view_frame_one_shot_runtime_probe", "runtime_probe", "run_probe", "probe_execution", "runtime_probe_executed"),
    ...
    "sampling_formula_movement": ("_effective_sample_fraction", "_sample_projected_frame", "sample_projected_frame", "adaptive_sampling_formula"),
    "alpha_compose_formula": ("alpha_compose", "alpha_compose_transparent", "compose_alpha", "alpha_compose_formula"),
    ...
    "real_ais_adsb_source": ("real_ais", "real_adsb", "ais_feed", "adsb_feed", "live_ais", "live_adsb"),
    "source_lineage_mutation": ("mutate_source_lineage", "source_lineage_mutated", "rewrite_source_identity", "source_identity_mutation", "source_lineage_write"),
    "hidden_as_missing_interpretation": ("hidden_as_missing", "hidden_is_missing", "missing_from_hidden", "hidden_missing_claimed"),
    "reduced_count_as_source_loss_interpretation": ("reduced_count_as_source_loss", "rendered_lower_than_visible_source_loss", "count_reduction_source_loss"),
    "occluded_as_source_loss_interpretation": ("occluded_as_source_loss", "occluded_as_source_lineage_loss", "source_deleted_by_occlusion"),
    "direct_c1_integration": ("direct_c1_integration", "c1_adapter", "c1_source_bridge", "connect_to_c1", "c3_to_c1_direct"),
    "c4_odoriba_bypass": ("c4_odoriba_bypass", "bypass_c4", "bypass_odoriba", "skip_odoriba_mediation"),
    "transparent_globe_leak_inference": ("transparent_globe_leak_inferred", "infer_transparent_globe_leak", "transparent_globe_leak_behavior"),
    ...
    "label_executable_reference": (
        "source_present_token", "source_label", "point_id", "source_lineage_integrity_token",
        "payload_identity_guard", "sampling_does_not_mutate_source",
        "presentation_count_does_not_mutate_source", "hidden_visibility_does_not_mutate_source",
        "mask_visibility_does_not_mutate_source", "occlusion_visibility_does_not_mutate_source",
        "reduced_count_is_not_source_loss", "hidden_is_not_missing", "occluded_is_not_source_lineage_loss",
        "controlled_raw_row_compatibility_seam", "developmental_compensation_surface",
        "future_c4_odoriba_handoff_material", "direct_c1_integration_not_authorized",
    ),
}
...
FORBIDDEN_IMPORT_PREFIXES = {
    ...
    "render_core.render_plan": "alpha_compose_formula",
...
SELF_TEST_SNIPPETS = {
    ...
    "hidden_as_missing_interpretation": "hidden_as_missing = True\n",
    "reduced_count_as_source_loss_interpretation": "reduced_count_as_source_loss = True\n",
    "occluded_as_source_loss_interpretation": "occluded_as_source_loss = True\n",
    ...
    "label_executable_reference": "def f():\n    return controlled_raw_row_compatibility_seam\n",
}

CLEAN_SYNTHETIC_CANDIDATE = '''
def build_dynamic_point_source_identity_contract_descriptor():
    return {
        "source_present_token": "source_present_token",
        "source_label": "source_label",
        "point_id": "point_id",
        "controlled_raw_row_compatibility_seam": "controlled_raw_row_compatibility_seam",
        "developmental_compensation_surface": "developmental_compensation_surface",
        "future_c4_odoriba_handoff_material": "future_c4_odoriba_handoff_material",
        "direct_c1_integration_not_authorized": "direct_c1_integration_not_authorized",
    }


def build_dynamic_point_source_lineage_integrity_descriptor():
    return {"guard": "source_lineage_integrity_token"}


def build_dynamic_point_sampling_source_guard_descriptor():
    return {"guard": "sampling_does_not_mutate_source"}


def build_dynamic_point_presentation_count_source_guard_descriptor():
    return {"guard": "presentation_count_does_not_mutate_source"}


def build_dynamic_point_hidden_visibility_source_guard_descriptor():
    return {"guard": "hidden_visibility_does_not_mutate_source"}


def build_dynamic_point_mask_occlusion_source_guard_descriptor():
    return {
        "mask": "mask_visibility_does_not_mutate_source",
        "occlusion": "occlusion_visibility_does_not_mutate_source",
        "reduced": "reduced_count_is_not_source_loss",
        "hidden": "hidden_is_not_missing",
        "occluded": "occluded_is_not_source_lineage_loss",
    }


def build_dynamic_point_source_lineage_guard_stop_line_ledger():
    return {"runtime_execution_authorized": False}


def dynamic_point_source_lineage_guard_boundary_descriptor():
    return {"boundary": "descriptor_contract_ledger_only"}


def dynamic_point_source_lineage_guard_planning_bundle():
...
def _family_for_name(name: str) -> str | None:
    ...
    if "hidden_as_missing" in lowered or "hidden_is_missing" in lowered or "missing_from_hidden" in lowered:
        return "hidden_as_missing_interpretation"
    if "reduced_count_as_source_loss" in lowered or "count_reduction_source_loss" in lowered:
        return "reduced_count_as_source_loss_interpretation"
    if "occluded_as_source_loss" in lowered or "occluded_as_source_lineage_loss" in lowered or "source_deleted_by_occlusion" in lowered:
        return "occluded_as_source_loss_interpretation"
    if "source_lineage_mutat" in lowered or "rewrite_source_identity" in lowered:
        return "source_lineage_mutation"
    if "direct_c1" in lowered or "connect_to_c1" in lowered or "c3_to_c1" in lowered:
        return "direct_c1_integration"
    ...
    if "real_ais" in lowered or "real_adsb" in lowered or "live_ais" in lowered or "live_adsb" in lowered:
        return "real_ais_adsb_source"
    return None
...
def _base_packet(target: str) -> dict[str, object]:
    return {
        ...
        "controlled_raw_row_compatibility_seam_label_allowed": "controlled_raw_row_compatibility_seam" in ALLOWED_STRING_LABELS,
