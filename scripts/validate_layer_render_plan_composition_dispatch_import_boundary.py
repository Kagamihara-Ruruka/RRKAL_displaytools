"""Validate import boundaries for a future composition dispatch helper module.
...
SCHEMA = "rrkal_displaytools.layer_render_plan_composition_dispatch_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/layer_render_plan_composition_dispatch.py")
...
def validate_source(source: str, source_name: str = "<memory>") -> dict[str, object]:
    ...
    return {
        ...
        "allowed_future_helper_behavior": "pure_dict_packet_labels_only",
...
def validate_target(path: Path) -> dict[str, object]:
    if not path.exists():
        return {
            ...
            "runtime_merge_enabled": False,
            "violations": [],
            "boundary": "Candidate composition dispatch helper module is not present yet; no target module was imported or executed.",
        }
...
def run_negative_self_test() -> dict[str, object]:
    snippets = {
        ...
        "pyais": "import pyais\n",
        "render_plan_module": "from render_core.render_plan import build_layer_render_plan_composition_apply_action\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate composition dispatch helper import boundaries.")
