"""Validate import boundaries for a future dynamic point payload/coordinate-quality helper.
...
SCHEMA = "rrkal_displaytools.dynamic_point_payload_coordinate_quality_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/dynamic_point_payload_coordinate_quality_boundary.py")
...
FORBIDDEN_NAME_PATTERNS = (
    ...
    "*hit*test*",
    "*controller*mutation*",
    "*controller*selection*dependency*",
...
FORBIDDEN_FAMILIES = {
    ...
    "cache_database_io": ["cache read", "cache write", "cache load", "cache save", "DB URL", "replay query", "database read"],
...
def _family_for_token(token: str) -> str:
    ...
    if any(word in lowered for word in ("websocket", "live ais", "live ads", "sql", "mysql", "stream")):
        return "live_source"
        ...
        packet.update(
            {
                ...
                "checked_imports": [],
                "checked_names": [],
                "boundary": "Candidate dynamic point payload/coordinate-quality helper module is not present yet; no target module was imported or executed.",
            }
        )
        return packet
