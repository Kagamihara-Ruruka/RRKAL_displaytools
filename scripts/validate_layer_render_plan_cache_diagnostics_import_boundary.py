"""Validate import boundaries for a future cache diagnostics helper module.
...
SCHEMA = "rrkal_displaytools.layer_render_plan_cache_diagnostics_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/layer_render_plan_cache_diagnostics.py")
...
def validate_source(source: str, source_name: str = "<memory>") -> dict[str, object]:
    ...
    return {
        ...
        "allowed_future_helper_behavior": "pure_cache_diagnostics_dict_list_scalar_logic_only",
        "cache_word_allowed_in_helper_names": True,
        "cache_lifecycle_modules_forbidden": True,
...
def validate_target(path: Path) -> dict[str, object]:
    if not path.exists():
        return {
            ...
            "cache_word_allowed_in_helper_names": True,
            "cache_lifecycle_modules_forbidden": True,
            "boundary": "Candidate cache diagnostics helper module is not present yet; no target module was imported or executed.",
        }
...
def run_negative_self_test() -> dict[str, object]:
    snippets = {
        ...
        "pyais": "import pyais\n",
        "render_plan_module": "from render_core.render_plan import build_layer_render_plan_cache_key\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate cache diagnostics helper import boundaries.")
