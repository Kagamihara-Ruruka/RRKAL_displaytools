"""Validate import boundaries for a future DataFrame normalizer helper.

This script is intentionally static. It parses Python source with ast and never
imports or executes the candidate module.
"""
...
def validate_source(source: str, source_name: str = "<memory>") -> dict[str, object]:
    ...
    return {
        "schema": "rrkal_displaytools.normalizer_import_boundary.v1",
        "source": source_name,
...
def validate_target(path: Path) -> dict[str, object]:
    if not path.exists():
        return {
            ...
            "runtime_merge_enabled": False,
            "violations": [],
            "boundary": "Candidate helper module is not present yet; no target module was imported or executed.",
        }
...
def run_negative_self_test() -> dict[str, object]:
    ...
    return {
        "schema": "rrkal_displaytools.normalizer_import_boundary.self_test.v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate normalizer helper import boundaries.")
    parser.add_argument("target", nargs="?", default="render_core/dataframe_normalizers.py")
