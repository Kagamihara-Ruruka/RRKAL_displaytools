...
$renderPlanComposeQueueSource = if (Test-Path -LiteralPath (Join-Path $RepoRoot "render_core\layer_render_plan_compose_queue.py")) {
    Get-Content -Raw -Encoding UTF8 (Join-Path $RepoRoot "render_core\layer_render_plan_compose_queue.py")
} else {
    ""
}
...
$renderPlanCacheDiagnosticsSource = if (Test-Path -LiteralPath (Join-Path $RepoRoot "render_core\layer_render_plan_cache_diagnostics.py")) {
    Get-Content -Raw -Encoding UTF8 (Join-Path $RepoRoot "render_core\layer_render_plan_cache_diagnostics.py")
} else {
    ""
}
$renderPlanExecutionPhaseTimingSource = if (Test-Path -LiteralPath (Join-Path $RepoRoot "render_core\layer_render_plan_execution_phase_timing.py")) {
    Get-Content -Raw -Encoding UTF8 (Join-Path $RepoRoot "render_core\layer_render_plan_execution_phase_timing.py")
} else {
    ""
}
...
$renderPlanCompiledReusedPacketsSource = if (Test-Path -LiteralPath (Join-Path $RepoRoot "render_core\layer_render_plan_compiled_reused_packets.py")) {
    Get-Content -Raw -Encoding UTF8 (Join-Path $RepoRoot "render_core\layer_render_plan_compiled_reused_packets.py")
} else {
    ""
}
$renderPlanResidualPacketSurfacesSource = if (Test-Path -LiteralPath (Join-Path $RepoRoot "render_core\layer_render_plan_residual_packet_surfaces.py")) {
    Get-Content -Raw -Encoding UTF8 (Join-Path $RepoRoot "render_core\layer_render_plan_residual_packet_surfaces.py")
} else {
    ""
}
$displayCoreSource = @(
...
$displayRuntimeImportBoundaryText = Invoke-CapturedNative powershell @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\check_display_runtime_import_boundary.ps1")
$displayRuntimeImportBoundary = $displayRuntimeImportBoundaryText | ConvertFrom-Json
if ($displayRuntimeImportBoundary.schema -ne "rrkal_displaytools.display_runtime_import_boundary_check.v1") {
    throw "Display runtime import boundary check schema missing"
}
if ($displayRuntimeImportBoundary.status -ne "pass") {
    throw "Display runtime import boundary check did not pass"
}
