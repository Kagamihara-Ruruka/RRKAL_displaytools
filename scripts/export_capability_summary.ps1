...
$summary = [ordered]@{
    ...
    current_capabilities = @(
        ...
        [ordered]@{
            ...
            description = "Layer render-plan performance now exposes runtime_pressure_snapshot, layer_render_state, lod_counters, heavy_overlay_defer_cache and runtime_optimization_review_summary metadata for target-FPS pressure, layer state, LOD and overlay defer/cache decisions; compression and RendererSkinAsset work stay out of displaytools."
        ...
        [ordered]@{
            ...
            description = "Runtime gate status, zero-diff parity evidence summary and parity artifact producer summary are visible in Qt, copy actions, diagnostics JSON, reviewer packets, handoff inspection and render-plan review packets while runtime merge and single-pass submission stay disabled until zero-diff parity."
