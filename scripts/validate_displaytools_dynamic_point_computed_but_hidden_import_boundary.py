"""Validate import boundaries for dynamic point computed-but-hidden helpers.
...
SCHEMA = "rrkal_displaytools.dynamic_point_computed_but_hidden_import_boundary.v1"
DEFAULT_TARGET = Path("render_core/dynamic_point_computed_but_hidden_boundary.py")
...
FORBIDDEN_FAMILIES: dict[str, tuple[str, ...]] = {
    ...
    "source_lineage_loss_interpretation": ("source_lineage_loss_claimed", "source_lineage_loss_interpretation", "occluded_source_lineage_loss", "source_deleted_by_occlusion"),
    ...
    "label_executable_reference": (
        "source_present_token", "computed_point_token", "hidden_visibility_token", "frame_visible_not_observed",
        "hidden_is_not_missing", "occluded_is_not_source_lineage_loss", "computed_but_hidden_contract",
        "transparent_globe_leak_not_inferred", "source_loss_not_inferred", "frame_buffer_read_blocked",
        "renderer_execution_blocked", "readiness_not_claimed",
    ),
}
...
SELF_TEST_SNIPPETS = {
    ...
    "source_lineage_loss_interpretation": "source_lineage_loss_claimed = True\n",
    ...
    "label_executable_reference": "def f():\n    return hidden_is_not_missing\n",
}

CLEAN_SYNTHETIC_CANDIDATE = '''
def build_dynamic_point_computed_but_hidden_contract_descriptor():
    return {
        "source_present_token": "source_present_token",
        "computed_point_token": "computed_point_token",
        "hidden_visibility_token": "hidden_visibility_token",
        "frame_visible_not_observed": "frame_visible_not_observed",
        "hidden_is_not_missing": "hidden_is_not_missing",
        "occluded_is_not_source_lineage_loss": "occluded_is_not_source_lineage_loss",
        "computed_but_hidden_contract": "computed_but_hidden_contract",
        "transparent_globe_leak_not_inferred": "transparent_globe_leak_not_inferred",
        "source_loss_not_inferred": "source_loss_not_inferred",
        "frame_buffer_read_blocked": "frame_buffer_read_blocked",
        "renderer_execution_blocked": "renderer_execution_blocked",
        "readiness_not_claimed": "readiness_not_claimed",
    }


def build_dynamic_point_hidden_visibility_contract_descriptor():
    return {"contract": "hidden_is_not_missing"}


def build_dynamic_point_computed_point_contract_descriptor():
    return {"contract": "computed_but_hidden_contract"}


def build_dynamic_point_computed_but_hidden_source_lineage_guard_descriptor():
    return {"guard": "occluded_is_not_source_lineage_loss"}


def build_dynamic_point_computed_but_hidden_stop_line_ledger():
    return {"frame": "frame_visible_not_observed"}


def dynamic_point_computed_but_hidden_boundary_descriptor():
    return {"boundary": "descriptor_contract_ledger_only"}


def dynamic_point_computed_but_hidden_planning_bundle():
...
def _family_for_name(name: str) -> str | None:
    ...
    if "source_lineage_loss" in lowered or "source_deleted_by_occlusion" in lowered:
        return "source_lineage_loss_interpretation"
