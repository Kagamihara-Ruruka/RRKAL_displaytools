...
def profile_ui_state_replay_packet(source: str) -> dict[str, object]:
    ...
    qt_inspector_actions = [
        ...
        ("layer_controls_guide", "Copy layer controls guide"),
...
class DisplayToolsQtPanel(QtWidgets.QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        source_map_button = QtWidgets.QPushButton("Inspect: Source map")
        ...
        for button, tooltip in (
            ...
            (source_map_button, "Replay/contracts: inspect render-plan compose source-map command and required monolith helpers."),
        ...
        source_map_button.clicked.connect(self.show_render_plan_compose_source_map)
    ...
    def decoupling_readiness_summary_text(self, packet: dict[str, object] | None = None) -> str:
        packet = packet if isinstance(packet, dict) else self.collect_decoupling_readiness()
        extraction_order = packet.get("first_extraction_order")
        extraction_order = extraction_order if isinstance(extraction_order, list) else []
    ...
    def layer_controls_guide_text(self) -> str:
        selected_layer = self.selected_layer_key or "none"
        selected_label = next(
            (text for key, text in LAYER_LABELS if key == self.selected_layer_key),
            selected_layer,
    ...
    def runtime_optimization_review_summary_text(self, packet: dict[str, object] | None = None) -> str:
        ...
        return (
            ...
            f"bridge_runtime_merge={packet.get('layer_state_precompute_compose_bridge_runtime_merge_enabled', '-')}; "
            f"bridge_submit={packet.get('layer_state_precompute_compose_bridge_single_pass_submission_enabled', '-')}; "
            f"submit_field={packet.get('layer_state_precompute_compose_bridge_single_pass_submission_field', '-')}; "
            f"zero_diff_parity={packet.get('layer_state_precompute_compose_bridge_zero_diff_parity_required', '-')}; "
            f"parity_field={packet.get('layer_state_precompute_compose_bridge_zero_diff_parity_field', '-')}; "
    ...
    def zero_diff_parity_evidence_summary_text(self) -> str:
        return (
            "Zero-diff parity evidence: "
            "artifacts=baseline_sequential_frame_rgba.png,merged_candidate_frame_rgba.png; "
            "visual_parity_passed=required; "
    ...
    def compose_parity_workflow_summary_text(self, packet: dict[str, object] | None = None) -> str:
        ...
        return (
            ...
            f"run={workflow.get('runner_command', '-')}; "
    ...
    def collect_render_plan_compose_source_map(self) -> dict[str, object]:
        return {
            "schema": "rrkal_displaytools.qt_render_plan_compose_source_map.v1",
            "status": "ready",
            "no_code_move": True,
            "script": "scripts\\inspect_render_plan_compose_source_map.ps1",
            "source_file": "taichi_global_bathymetry.py",
            "target_module": "render_core/render_plan.py",
            "required_targets": [
                "alpha_compose",
                "alpha_blend_compose",
                "layer_render_plan_compose_queue",
                "layer_render_plan_compose_runs",
                "apply_layer_render_plan_composition",
                "compile_layer_render_plan",
            ],
            "next_action": "Run the source-map inspector after the formal post-7 gate and before moving render-plan helpers.",
        }

    def show_render_plan_compose_source_map(self) -> None:
        self.command_text.setPlainText(json.dumps(self.collect_render_plan_compose_source_map(), ensure_ascii=False, indent=2))
        self.status.setText("Displayed render-plan compose source-map JSON")
    ...
    def copy_layer_controls_guide(self) -> None:
